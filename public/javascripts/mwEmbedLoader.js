...
function kDoIframeRewriteList(rewriteObjects) {
    for (var i = 0; i < rewriteObjects.length; i++) {
        var options = {
            'width': rewriteObjects[i].width,
            'height': rewriteObjects[i].height
        };
...
function kalturaIframeEmbed(replaceTargetId, kEmbedSettings, options) {
    ...
    console.log(mw.getConfig('EmbedPlayer.EnableIframeApi'));
...
function kDirectDownloadFallback(replaceTargetId, kEmbedSettings, options) {
    var targetNode = document.getElementById(replaceTargetId);
    if (!targetNode) {
        if (console && console.log) console.log("Error could not find object target: " + replaceTargetId);
    }
    try {
        targetNode.innerHTML = '';
    } catch(e) {}
    if (!options) options = {};
    if (!options.width && kEmbedSettings.width) options.width = kEmbedSettings.width;
    if (!options.height && kEmbedSettings.height) options.height = kEmbedSettings.height;
    if (!options.width && targetNode.style.width) options.width = targetNode.style.width;
    if (!options.height && targetNode.style.height) options.height = targetNode.style.height;
    if (!options.height) options.height = 300;
    if (!options.width) options.width = 400;
    var downloadUrl = SCRIPT_LOADER_URL.replace('ResourceLoader.php', 'modules/KalturaSupport/download.php') + '/wid/' + kEmbedSettings.wid;
    ...
    var ddHTML = '<div id="' + ddId + '" style="width: ' + options.width + ';height:' + options.height + ';position:relative">' + '<img style="width:100%;height:100%" src="' + thumbSrc + '" >' + '<a href="' + downloadUrl + '" target="_blank" style="' + playButtonCss + '"></a>' + '</div>';
    var parentNode = targetNode.parentNode;
        ...
        window['SWFObject'].prototype['write'] = function(targetId) {
            ...
            kAddReadyHook(function() {
                ...
                if (kIsHTML5FallForward() && kEmbedSettings.uiconf_id) {
                    ...
                    if (kEmbedSettings.uiconf_id) restoreKalturaKDPCallback();
...
function kCheckAddScript() {
    if (mw.getConfig('Kaltura.EnableEmbedUiConfJs') && !mw.getConfig('Kaltura.UiConfJsLoaded') && !mw.getConfig('EmbedPlayer.IsIframeServer')) {
        ...
        for (var i = 0; i < playerList.length; i++) {
            kAppendScriptUrl(baseUiConfJsUrl + kEmbedSettingsToUrl(playerList[i].kEmbedSettings),
                function() {
                    requestCount--;
                    if (requestCount == 0) {
                        kCheckAddScript();
    ...
    if (!mw.getConfig('Kaltura.ForceFlashOnDesktop') && (mw.getConfig('Kaltura.LoadScriptForVideoTags') && kPageHasAudioOrVideoTags())) {
        kAddScript();
        return;
    }
...
function kIsHTML5FallForward() {
    ...
    if (navigator.userAgent.indexOf('Android 2.') != -1) {
        if (mw.getConfig('EmbedPlayer.UseFlashOnAndroid') && kSupportsFlash()) {
            return false;
        } else {
            return true;
        }
...
function kAddScript(callback) {
    ...
    jsRequestSet.push('mwEmbed', 'mw.Uri', 'mw.style.mwCommon', 'mw.EmbedPlayer', 'mw.MediaElement', 'mw.MediaPlayer', 'mw.MediaPlayers', 'mw.MediaSource', 'mw.EmbedTypes', 'mw.style.EmbedPlayer', 'mw.PlayerControlBuilder', 'mw.PlayerSkinMvpcf', 'mw.style.PlayerSkinMvpcf', 'mw.EmbedPlayerNative', 'mw.EmbedPlayerKplayer', 'mw.EmbedPlayerJava', '$j.ui', '$j.widget', '$j.ui.mouse', '$j.fn.hoverIntent', '$j.cookie', 'JSON', '$j.ui.slider', '$j.fn.menu', 'mw.style.jquerymenu', 'mw.TimedText', 'mw.style.TimedText');
    if (mw.getConfig('EmbedPlayer.IsIframeServer')) {
    ...
    if (kIsHTML5FallForward() || objectPlayerList.length) {
        jsRequestSet.push('MD5', "mw.KApi", 'mw.KWidgetSupport', 'mw.KAnalytics', 'mw.KDPMapping', 'mw.KAds', 'mw.AdTimeline', 'mw.BaseAdPlugin', 'mw.AdLoader', 'mw.VastAdParser', 'mw.KCuePoints', 'mw.KTimedText', 'mw.KLayout', 'mw.style.klayout', 'titleLayout', 'playlistPlugin', 'controlbarLayout', 'faderPlugin', 'watermarkPlugin', 'adPlugin', 'captionPlugin', 'bumperPlugin');
        jsRequestSet.push('mw.Playlist', 'mw.style.playlist', 'mw.PlaylistHandlerMediaRss', 'mw.PlaylistHandlerKaltura', 'mw.PlaylistHandlerKalturaRss');
    }
    kLoadJsRequestSet(jsRequestSet, callback);
}
function kIsIE() {
...
function kGetKalturaPlayerList() {
    ...
    for (var i = 0; i < objectList.length; i++) {
        ...
        if (objectList[i].getAttribute('data')) {
            if (tryAddKalturaEmbed(objectList[i].getAttribute('data'), flashvars)) continue;
        }
    }
...
function kGetEntryThumbUrl(entry) {
    ...
    return kCdn + '/p/' + entry.partner_id + '/sp/' + entry.partner_id + '00/thumbnail/entry_id/' + entry.entry_id + '/width/' + entry.width + '/height/' + entry.height;
}
...
window.checkUserAgentPlayerRules = function(ruleSet, getMsg) {
    ...
    for (var i in ruleSet.rules) {
        ...
        if (rule.match) {
            ...
            if (ua.match(eval(rule.regMatch))) return getAction(i);
        }
    }
    return 'flash';
};
