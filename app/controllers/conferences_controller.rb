...
class ConferencesController < ApplicationController
  ...
  def join
    if authorized_action(@conference, @current_user, :join)
      unless @conference.valid_config?
        flash[:error] = t(:type_disabled_error, "This type of conference is no longer enabled for this Jiaoxuebang site")
  ...
  def require_config
    unless WebConference.config
      flash[:error] = t('#conferences.disabled_error', "Web conferencing has not been enabled for this Jiaoxuebang site")
      redirect_to named_context_url(@context, :context_url)
    end
  end
