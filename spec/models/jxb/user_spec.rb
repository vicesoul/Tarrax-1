...
describe User do
  context "course systems" do
    before :each do
      course_model
      @account = Account.default
    end

    it "should find lower rank" do
      job_position_with_user
      cs1 = course_system_model :rank => 'mandatory'
      cs2 = course_system_model :rank => 'recommended'

      @user.course_systems_for_account(@account).should == [cs1]
    end

    it "should find lower rank with subaccount" do
      job_position_with_user
      cs1 = course_system_model :rank => 'optional'

      sub_account = account_model :parent_account => @account
      cs2 = course_system_model :rank => 'recommended', :account => sub_account

      @user.course_systems_for_account(@account).should == [cs1]
    end

    it "should skip duplicated courses" do
      job_position_with_user
      cs1 = course_system_model :rank => 'optional'

      sub_account = account_model :parent_account => @account
      cs2 = course_system_model :rank => 'optional', :account => sub_account
